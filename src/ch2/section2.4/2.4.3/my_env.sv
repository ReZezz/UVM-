...
class my_env extends uvm_env;
    ...
    virtual function void build_phase(uvm_phase phase);               //uvm的创建阶段,此时有两个build_phase，执行顺序是先执行树根my_env的build_phase，再执行树叶
        ...
        uvm_config_db#(uvm_object_wrapper)::set(this,                 //使用default_sequence自动启动sequence，第一个参数是uvm_test_top,由于这里是env已经是uvm_test_top了，所以这里写this
                                        "i_agt.sqr.main_phase",       //第二个参数是相对于第一个参数的相对路径，而第一个参数是this所以第二个参数不需要写uvm_test_top直接写i_agt.sqr.main_phase就行
                                        "default_sequence",           //第三第四个参数以及uvm_object_wrapper是uvm规定这么写的
                                        my_sequence::type_id::get());
                                                                      //虽然config_db通常成对出现，但是在启动sequence时，不需要成对出现，只用写set不用写get
...
//    my_sequence seq;
//    phase.raise_objection(this);                                      //driver失去对objection的控制权之后，控制权交给sequence
//    seq = my_sequence::type_id::create("seq");                         
//    seq.start(i_agt.sqr);                                             //start任务的参数是一个sequencer指针，sequence将产生的transaction交给这个sequencer指针指向的sequencer
