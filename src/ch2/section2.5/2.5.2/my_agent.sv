...
function void my_agent::connect_phase(uvm_phase phase);                  //在agent内部，将monitor的端口连接到agent的外壳端口上
    ...
    if(is_active == UVM_ACTIVE) begin                                    //如果是i_agent，那么里面有driver和sequencer，需要把driver和sequencer连接起来一个数据通道
        drv.seq_item_port.connect(sqr.seq_item_export);                  //连接好之后就可以在driver中使用try_next_item或者get_next_item任务向sequencer申请新transaction了
    end
