...
task my_scoreboard::main_phase(uvm_phase phase);
    ...
    super.main_phase(phase);                                                                //之所以要用并行的while是因为exp_port.get(get_expect)这个是阻塞任务，fifo里没数据就会卡住
    fork                                                                                    //里面有两个while线程，这两个线程是并行运行的
        ...
        while(1)begin
            ...
            if(expect_queue.size() > 0)begin
                ...
                result = get_actual.compare(temp_tran);                                     //把弹出来的那个包做对比,使用uvm_field宏的函数完成，简化代码
                if(result)begin
                    ...
                    temp_tran.print();                                                      //使用uvm_field宏的函数完成，简化代码
                    $display("the actual pkt is");
                    get_actual.print();                                                     //使用uvm_field宏的函数完成，简化代码  
                end
            end
            else begin
