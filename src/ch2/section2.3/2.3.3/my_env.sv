...
class my_env extends uvm_env;
    ...
    my_monitor i_mon;                                                 //创建一个my_monitor的句柄i_mon                                                 
    my_monitor o_mon;                                                 //创建一个my_monitor的句柄o_mon
    ...
    virtual function void build_phase(uvm_phase phase);               //uvm的创建阶段,此时有两个build_phase，执行顺序是先执行树根my_env的build_phase，再执行树叶
        ...
        i_mon = my_monitor::type_id::create("i_mon",this);            //调用uvm自动创建一个my_monitor的实例名字叫i_mon，父节点是this指针指向的my_env
        o_mon = my_monitor::type_id::create("o_mon",this);            //调用uvm自动创建一个my_monitor的实例名字叫o_mon，父节点是this指针指向的my_env
    endfunction
