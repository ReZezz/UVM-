...
initial begin                                        //这里因为加入了env机制，所以树根变成了my_env而不再是my_driver所以run_test的参数也相应变成my_env了
    run_test("my_env");                              //由于加入了factory机制，所以不需要做实例化，也不需要对main_phase显式调用了,同时也没有显示调用finish语句结束仿真
end
...
initial begin                                        //第一个参数null是父类，这里是top所以写null，"uvm_test_top"是run_test产生的实例，第三个参数可以理解成要传到哪里，第四个参数是要传的东西
    uvm_config_db#(virtual my_if)::set(null, "uvm_test_top.drv", "vif", input_if);
                                                     ...
                                                     //::set:我要发送数据     null:发送者是顶层（top），没有父节点，写 null
                                                     //"uvm_test_top.drv":接收的是test组件的drv,也就是my_env的drv实例
                                                     //"vif":给这个数据起个名字，叫vif   
